...
<link href="CSS/style.css" rel="stylesheet">
		...
		function checkreader(form){
			if(form.barcode.value==""){
				...
				form.barcode.focus();
				return;
			}else if(form.barcode.value.length!=9){
				alert("输入读者编号格式不正确!");
				form.barcode.focus();
				return;
			}
		...
		function checkbook(form){
			if(form.barcode.value==""){
				alert("读者编号不能为空!");form.barcode.focus();return;
			}else if(form.barcode.value.length!=9){
				alert("输入读者编号格式不正确!");form.barcode.focus();return;
			}
			if(form.readername.value==""){
				alert("请先查询出详细的读者信息!");form.barcode.focus();return;
			}
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
                  ...
                  <tr>
                    <td align="center"><table width="96%" border="0" cellpadding="0" cellspacing="0">
                        <tr>
                          <td height="27">姓&nbsp;&nbsp;&nbsp;&nbsp;名：
                            <input name="readername" type="text" id="readername" value="<%=name%>" disabled></td>
                          <td>性&nbsp;&nbsp;&nbsp;&nbsp;别：
                            <input name="sex" type="text" id="sex" value="<%=sex%>" disabled></td>
                          <td>读者类型：
                            <input name="readerType" type="text" id="readerType" value="<%=typename%>"disabled></td>
                        </tr>
                        <tr>
                          <td height="27">证件类型：
                            <input name="paperType" type="text" id="paperType" value="<%=paperType%>" disabled></td>
                          <td>证件号码：
                            <input name="paperNo" type="text" id="paperNo" value="<%=paperNO%>"disabled></td>
                          <td>可借数量：
                            <input name="number" type="text" id="number" value="<%=number%>" size="17" disabled>
                            册
                            &nbsp;</td>
                        </tr>
                    </table></td>
