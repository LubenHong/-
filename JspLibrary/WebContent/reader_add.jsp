...
function check(form){
	if(form.name.value==""){
		alert("读者姓名不能为空!");form.name.focus();return false;
	}
	if(form.barcode.value==""){
		alert("读者编号不能为空!");form.barcode.focus();return false;
	}else if(form.barcode.value.length!=9){
		alert("请输入正确的读者编号！");form.barcode.focus();return false;
	}
	if(form.birthday.value==""){
		alert("读者出生日期不能为空！");form.birthday.focus();return false;
	}
	if(form.paperNO.value==""){
		alert("证件号码不能为空!");form.paperNO.focus();return false;
	}else if(form.paperNO.value.length!=18){
		alert("请输入正确的证件号码!");form.paperNO.focus();return false;
	}
	if(form.tel.value!=""&&form.tel.value.length!=11){
		alert("请输入正确的电话号码!");form.tel.focus();return false;
	}
	if(form.email.value!=""&&(!/(\S)+[@]{1}(\S)+[.]{1}(\w)+/.test(form.email.value))) 
	{   form.email.value=""; 
...
<%}%> 
      ...
      <tr>
        ...
        <td><input name="birthday" type="text" id="birthday">*(格式为：1990-01-01)</td>
      </tr>
      <tr>
